...
\begin{definition}[Descent datum]
	Let $(\CatC, \mathcal{J})$ be a site, $F: \CatC\to \Cat$ a pseudo functor and $U \to X$ be a covering. A descent datum for $F$ along $U \to X$ consists of
	\begin{enumerate}
		\item an object $x\in F(U)$,
		\item an isomorphism $\alpha:d_0(x) \xrightarrow{\sim} d_1(x)$ in $F(U\times_X U))$ between two pullbacks of $x$ to
		$U\times_X U$
		\item a cocycle condition $d_2(\alpha)\circ d_0(\alpha) = d_1(\alpha)$ in $F(U\times_X U\times_X U)$, i.e. the following commutes:
...
\begin{remark}
	This definition specializes to descent data for rings, modules,
	quasi-coherent sheaves, schemes, and stacks.
\end{remark}
...
\begin{theorem}[Faithfully flat descent for modules]
	...
	\]
	is an equivalence of categories.
	Explicitly, it sends an $R$-module $M$ to the descent datum
	\[
	M \longmapsto (S \otimes_R M, \alpha_M),
	\]
	where $\alpha_M$ is the canonical isomorphism
	\[
	(S \otimes_R M)\otimes_R S \;\cong\; S \otimes_R (S \otimes_R M).
	\]
\end{theorem}
...
\begin{definition}[Descent category for quasi-coherent sheaves]
	Let $f : U \to X$ be a covering. The category 
	\[
	\Desc_{\QCoh}(U/X)
	\]
	of descent data for quasi-coherent sheaves along $U \to X$ is defined as follows.
	
	\textbf{Objects:}
	Pairs $(\mathcal{F}, \varphi)$ consisting of
	\begin{itemize}
		\item a quasi-coherent sheaf $\mathcal{F}$ on $U$,
		\item an isomorphism
		\[
		...
		on $U \times_X U$,
	\end{itemize}
	such that the cocycle condition holds on $U \times_X U \times_X U$:
	\[
	...
	A morphism
	\[
	(\mathcal{F}, \varphi) \longrightarrow (\mathcal{F}', \varphi')
	\]
	is a morphism of quasi-coherent sheaves
	\[
	\psi : \mathcal{F} \to \mathcal{F}'
	\quad \text{in } \QCoh(U)
	\]
	such that the following diagram commutes on $U \times_X U$:
	\[
	\begin{tikzcd}
		\pi_1^*\mathcal{F}
		\arrow[r, "\varphi"]
