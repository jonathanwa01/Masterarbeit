...
\begin{definition}[Descent datum]
	...
	\begin{enumerate}
		...
		\item an isomorphism $\alpha$ between the two pullbacks of $\xi$ to
		$U\times_X U$,
...
\begin{proposition}[The totalization as descent data]
	\label{prop:tot-mod-desc}
	Let $R\to S$ be a ring homomorphism and set
	\[
	S^n:=\underbrace{S\otimes_R\cdots\otimes_R S}_{n+1\text{ factors}}.
	\]
	The cosimplicial $R$-algebra $S^\bullet$ induces a cosimplicial diagram of
	categories
	% https://q.uiver.app/#q=
	\[\begin{tikzcd}
		{\Mod_S} & {\Mod_{S\otimes_R S}} & {\Mod_{S\otimes_R S\otimes_R S}} & \cdots
		\arrow[shift left, from=1-1, to=1-2]
		\arrow[shift right, from=1-1, to=1-2]
		\arrow[shift right=2, from=1-2, to=1-3]
		\arrow[shift left=2, from=1-2, to=1-3]
	...
	\[
	\Tot(\Mod_{S^\bullet})\cong \Desc(S/R),
	\]
	where $\Desc(S/R)$ is the category of descent data for modules relative to
	$R\to S$.
\end{proposition}

\begin{proof}
	Write $\pi_1, \pi_2: S\to S\otimes_R S$ for the two morphisms $s\mapsto s\otimes 1$ and $s\mapsto 1\otimes s$ resp. For an $S$ module $N$ write $\pi_i^*N = N\otimes_{S, \pi_i} (S\otimes_R S)$ for the $S\otimes S$ module induced by $\pi_i$.
	An object of the totalization is an object
	\[
	N\in \Mod_S
	\]
	together with an isomorphism between its two pullbacks to
	$\Mod_{S\otimes_R S}$,
	\[
	\alpha:\pi_1^*N\xrightarrow{\sim}\pi_2^*N,
	\]
	satisfying the compatibility condition after pulling back to
	$S\otimes_R S\otimes_R S$. Explicitly this condition is
	\[
	\pi_{23}^*(\alpha)\circ \pi_{12}^*(\alpha)=\pi_{13}^*(\alpha).
	\]
	Under the identification $\pi_1^*N \cong N\otimes_R S$ and $\pi_2^*N \cong S\otimes_R N$ as $S\otimes_R S$-modules, $\alpha$ is precisely an $S\times_R S$-linear morphism $N\otimes_R S\to S\otimes_R N$ satisfying the cocycle compatibility over $S\otimes_R S\otimes_R S$.
	This is precisely an object of $\Desc(S/R)$.
	
	Similarly, a morphism in the totalization is an $S$-linear map
	$f:N\to N'$ such that the square
	\[
	\begin{tikzcd}
	...
	\end{tikzcd}
	\]
	commutes. This is exactly the definition of a morphism in $\Desc(S/R)$.
	Hence
	\[
	\Tot(\Mod_{S^\bullet})\simeq \Desc(S/R).
	\]
\end{proof}

\begin{remark}
	Since $\Mod_{S^\bullet}$ takes values in the strict $2$-category $\Cat$, by \ref{lem:truncated-tot-for-stric-n-cats} the totalization is equivalent to the $2$-truncated totalization, i.e. the descent category $\Desc$.
\end{remark}

\begin{theorem}[Faithfully flat descent for modules]
	\label{thm:faithfully-flat-descent-modules}
	(\cite[Theorem 3.16]{khan2022stacks})
	Let $R\to S$ be faithfully flat. Then the canonical functor
	\[
	\Mod_R\longrightarrow \Tot(\Mod_{S^\bullet})
	\]
	is an equivalence of categories. Under the identification
	\[
	\Tot(\Mod_{S^\bullet})\simeq \Desc(S/R),
	\]
	...
	\]
	where $\alpha_M$ is the canonical descent datum.
\end{theorem}
...
\begin{proof}
	...
	\[
	\Mod_A \cong \Tot\big(\Mod_{B^\bullet}\big),
	\]
