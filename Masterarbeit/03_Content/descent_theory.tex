...
\begin{theorem}[Fpqc descent for affine schemes]
	...
	\[
	\underline{\Spec}_X : \QCohAlg(X)^{op} \xrightarrow{\simeq} \Aff/X.
	\]
\end{theorem}
...
\begin{proof}
	...
	\]
	in $\QCoh(U)$, any descent datum for $\QCohAlg_{(-)}$ is in
	particular a descent datum for $\QCoh_{(-)}$ carrying compatible
	...
	equivalence
	\[
	\AffOver{(-)}\xrightarrow{\;\sim\;} \Desc(U/X). \qedhere
	\]
\end{proof}
